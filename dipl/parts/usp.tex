...
\subsection{\textit{USP-OR-A}}
    ...
    \begin{minipage}{0.45\textwidth}  
	    ...
	    \captionof{figure}{Changing of $\tau_{\mathcal{A}}$ with increased number of stations in \textit{cpza} dataset. $\mathcal{A}$ was obtained using algorithm we will talk about later}
    ...
    \begin{minipage}{0.45\textwidth} 
	    ...
	    \captionof{figure}{Changing of $\gamma_{\mathcal{A}}$ with increased number of stations in \textit{cpza} dataset. $\mathcal{A}$ was obtained using algorithm we will talk about later}
	    \label{plot:angamma-cpza-size}   
    \end{minipage}
    }
	\end{figure}
	
	On a query from $x$ at time $t$ to $y$, we first perform the \textit{local front search} (see algorithm~\ref{alg:uspora-query}). In this step we explore the neighbourhood of $x$ with a time-dependent Dijkstra's algorithm, which takes on average time $\mathcal{O}(\sqrt{r_{2}n} (\log (\sqrt{r_{2}n}) + \delta))$. We then expand all the USPs between $u$ and $v$ such that $u \in lan(x)$ and $v \in blan(y)$, which takes on average $\mathcal{O}(r_{3}^{2} \tau_{\mathcal{A}} \gamma_{\mathcal{A}})$. Finally, from each $v \in blan(y)$ we do a time-dependent Dijkstra's algorithm, restricted to $bneigh(y)$, leading to time complexity $\mathcal{O}(r_{3}\sqrt{r_{2}n} (\log (\sqrt{r_{2}n}) + \delta))$.
	...
	\begin{proof}
		...
		In the \textit{local back search} we run a time-dependent Dijkstra search from all back LANs of $y$. And since this algorithm is exact and starts from each back LAN as early as possible, we get the optimal connection to $y$. \\
	...
	\subsubsection{Choosing ANs based on node properties}
		...
		\begin{enumerate}
			...
			\item High \textbf{betweenness centrality}. Betweenness centrality for a node $v$ is defined as 
		...
		\noindent We define similarly $\bm{k_{bc}^{avg}}$ and $\bm{k_{bc}^{max}}$.
		
	\subsubsection{Choosing ANs heuristically - \textit{locsep}}