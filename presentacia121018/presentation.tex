    ...
    \end{frame}
        ...
        \begin{frame}
            ...
            \begin{itemize}
                ...
                \item<2-> \textbf{Timetable} - \emph{cestovnı poriadok} nad danım podkladovım grafom. Mnoina elementárnych spojení
        ...
        \end{frame}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    \item<1-> \textbf{Underlying graph} - \emph{podkladovı graf} (mapa)
                    \item<1-> \textbf{Timetable} - \emph{cestovnı poriadok} nad danım podkladovım grafom. Mnoina elementárnych spojení
                    ...
                    \begin{itemize}
                        \item<2-> Inı poh¾ad: ceny hrán sú \emph{po èastiach lineárne} funkcie (piecewise linear functions) udávajúce cenu hrany v kadom èasovom okamihu ~\cite{engineeringroute09}
                    \end{itemize}
                \end{itemize}
            \end{frame}
        ...
        \begin{frame}
            ...
            \begin{itemize}
                ...
                \end{itemize}
                \item<3-> Avšak napr. celoeurópska sie elezníc má rádovo 100000 vrcholov $\rightarrow$ cestovnı poriadok nad takouto sieou mnohonásobne viac.
                \item<4-> Na queries má server \emph{odpoveda on-line}. Potenciálne ve¾a queries za èasovú jednotku.
            \end{itemize}
        \end{frame}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \begin{figure}[h]
                            ...
                            \end{center}
                            \caption{\label{fig:apxdocompromises} Moné kompromisy DO Thorupa a Zwicka}
                        \end{figure}
                        }
                    \end{itemize}
                \end{itemize}
...
%---------------------------------------------------------------------
    ...
    \begin{frame}
        ...
        \begin{itemize}
            ...
            \item<3-> Pre grafy so elanou vlastnosou existuje efektívny algoritmus na h¾adanie najkratších ciest $\rightarrow$ vieme ho \emph{upravi pre cestovné poriadky nad tımto grafom}?
    ...
    \end{frame}
        ...
        \subsection{Highway dimension}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    \item<1-> \textbf{Highway dimension} (intuícia) - graf má malú HD, ak pre oblas kadej ve¾kosti máme malú mnoinu vrcholov, cez ktoré prechádzajú všetky dostatoène dlhé najkratšie cesty v grafe.
            ...
            \begin{frame}
                \frametitle{Malá HD}
                \begin{itemize}
                    \item<1-> Malá HD \emph{garantuje nízku èasovú zloitos} queries viacerıch \emph{exaktnıch} DO metód, ktoré navyše majú rıchle predspracovanie a ve¾kos DO iba o èosi väèšiu ako vstup ~\cite{highwaydim10}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \item<6-> \textbf{Time-dependent CH} \cite{timedepch09} zrıchlenie cca. ako SHARC, ve¾ká pamäová nároènos
                    \end{itemize}
                \end{itemize}
            \end{frame}
        ...
        \subsection{r(n)-separator}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    \item<1-> \textbf{r(n)-separátor} ~\cite{distlabel04} - graf má r(n)-separátor ak v òom existuje mnoina vrcholov $S$, $|S| \leq r(n)$, ktorej odstránením sa graf rozpadne na kompenenty ve¾kostí $\leq 2n/3$, z ktorıch kadı má r(2n/3)-separátor
                \end{itemize}
                \uncover<2->{
                    \begin{figure}[h]
                        \scriptsize
                        ...
                        \end{center}
                        \caption{\label{fig:separator} Graf s $3$-separátorom (èie $r(n)$ je konštanta v tomto prípade)}
                    \end{figure}
                }
            \end{frame}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \item<6-> Napr. pre planárne grafy je èas predspracovania $\mathcal{O} (n^{3/2} \log n)$
                    \end{itemize}
                \end{itemize}
            \end{frame}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \item<4-> V rámci separátora kadı s kadım
                    \end{itemize}
                \end{itemize}
            \end{frame}
        ...
        \subsection{Scale-free network}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \item<3-> $Pr[\deg(v) = x] \propto C \cdot x^{-\tau}$
                        \item<4-> $C$ je konštanta, $\tau \in (2, 3)$
                    \end{itemize}
                    \item<5-> Skoro vdy \textbf{small-world} - oèakávanı poèet skokov pre náhodnú dvojicu vrcholov $\propto \log n$
                    \uncover<6->{
                        \begin{figure}[h]
                            \scriptsize
                            \begin{center}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    \item<1-> Dve metódy, obe \emph{nie úplne exaktné}, obe zaloené na vıznaènıch bodoch
                    \item<2-> \textbf{Sketch-based DO} ~\cite{sketchbased10}
                    \begin{itemize}
                        \item<3-> Náhodné vybratie vıznaènıch bodov (\textbf{sampling})
                        \item<4-> Pre kadı bod urèenie vzdialeností do vıznaènıch bodov $\rightarrow$ \textbf{sketch} (\emph{èrta}) vrchola
        ...
        \subsection{Vıskum}
            ...
            \subsubsection*{Vlastnosti cestovnıch poriadkov}
            \begin{frame}
                \frametitle{Vlastnosti cestovnıch poriadkov}
                \begin{itemize}
                    \item<1-> Vlastnosti cestovnıch poriadkov budú \emph{zálea} od toho:
                    \begin{itemize}
                        \item<2-> \emph{Nad akım grafom} je danı cestovnı poriadok
                        \item<3-> \emph{Akého typu} je sám cestovnı poriadok (pravidelná štruktúra, rôzna frekvencia liniek v rôznych oblastiach)
                    \end{itemize}
                    \item<4-> ``Divokım'' cestovnım poriadkom vieme ¾ahko pokazi peknú štruktúru podkladového grafu
                        \item<5-> Vo všeobecnosti sa iadna z vlastností (HD, r(n)-separátor, bezškálovos, planarita) nezachová
                    \item<6-> Inkrementálne povolova èoraz zloitejšie cestovné poriadky - pokia¾ sa a dá ís aby sa vlastnosti zachovávali a vıpoèet bol efektívny
                \end{itemize}
            \end{frame}
            ...
            \begin{frame}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \item<3-> Na èasovo expandované grafy mono vieme \emph{upravi algoritmy}
                    \end{itemize}
                \end{itemize}
            \end{frame}
...
%---------------------------------------------------------------------
    ...
    \end{frame}
        ...
        %---------------------------------------------------------------------
        %   Experimentálne zisovanie vlastností
        %---------------------------------------------------------------------
        \subsection{Experimentálne zisovanie vlastností}
        \begin{frame}
            \frametitle{Experimentálne zisovanie vlastností}
            \begin{itemize}
                \item<1-> \emph{Experimentálne zisovanie vlastností} grafov pre rôzne typy cestovnıch poriadkov
                \begin{itemize}
                    \item<2-> Náhodne generované cestovné poriadky nad cestnou sieou
                    \begin{itemize}
                        \item<3-> Dáta z Open street maps
                        \item<4-> Dáta PTV AG \cite{engineeringroute09}
                        \item<5-> Husté/riedke/vybalancované/pravidelné/nepravidelné cestovné poriadky...
                    \end{itemize}
                    \item<6-> Ako zisova efektívne HD?
                \end{itemize}
                \item<7-> C++ - kód
                \item<7-> Python - parsovanie vstupov
            \end{itemize}
        \end{frame}
        ...
        \begin{frame}
            ...
            \begin{itemize}
                ...
                \item<4-> Rôzne dáta:
                \begin{itemize}
                    \item<5-> Náhodné generovanie nad mapou Slovenska
                    \item<6-> Medzimestské spoje v okolí iliny (cp.sk)
                    \item<7-> Cestovné poriadky SR
                    \item<8-> Náhodné cestovné poriadky pre London Underground
                    \item<9-> Cestovné poriadky leteckıch liniek - United Airlines, Delta Airlines
                \end{itemize}
                \item<10-> C++ - kód
                \item<10-> Python - parsovanie vstupov
            \end{itemize}
        \end{frame}
