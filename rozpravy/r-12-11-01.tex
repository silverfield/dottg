...
%   various settings
%---------------------------------------------------------------------
\pgfrealjobname{r-12-10-31} % <-- NOTE: this needs to be the real document's basename
...
\begin{document}
	...
	\begin{table}[h]{
		...
		\hline
			\rowcolor{tablehead}
           	\textbf{Name} & \textbf{Type of network} & \textbf{Provided by} & \textbf{\# of stations} & \parbox{2.5cm}{\textbf{\# of elementary connections}} & \textbf{Time range} & \textbf{Note} \\
        %data
        \hline
    ...
    \begin{definition}
        ...
        \begin{itemize}
        		...
        		\item \textbf{Size (length)} of the connection is $e_{q}^{k} - e_{p}^{1}$.        
        \end{itemize}
    \end{definition}
    
    Overtaking, simply stated, means that there are connections in the timetable that overtake one another - that is by choosing a later connection, we will actually arrive sooner to the destination.
    ...
    Informally, express lines are connections that bypass several stops on the way. In our definition of timetable graph, we do not allow such connections, but we can extend the definition.
    ...
    %---------------------------------------------------------------------    
    \section{Adjustment of Gavoille's algorithm for graphs with $r(n)$ separator ~\cite{distlabel04}}
    ...
    \begin{itemize}
    		\item Adjustment of Gavoille's algorithm - formally
    		\item United airlines and road network of Slovakia - extract data
    		\item Start work on the diagnostic program
    		\item Replace null lengths in underlying graphs of existing timetables
    \end{itemize}
